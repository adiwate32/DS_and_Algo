...
public class Batches {
    public int solve(int A, ArrayList<Integer> B, ArrayList<ArrayList<Integer>> C, int D) {
        ...
        for (ArrayList<Integer> integers : C) {
        ...
        for(int i=1; i<=A; i++)
        {
            if(!vis[i])
            {
                if(dfs(0, graph, i, vis, B) >= D) ans++;
            }
        }
        return ans;
    }

    private int dfs(int cnt, List<List<Integer>> graph, int src, boolean[] vis, ArrayList<Integer> B)
    {
        vis[src] = true;
        cnt += B.get(src-1);
