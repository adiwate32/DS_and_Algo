...
public class Combination_Sum_II {
    ...
    public ArrayList<ArrayList<Integer>> combinationSum(ArrayList<Integer> A, int B) {
        Collections.sort(A);
        backTracking(A, B, 0, new ArrayList<>());
        Collections.sort(ans, (a, b) -> {
            for(int i=0; i< Math.min(a.size(), b.size()); i++)
            {
                if(!Objects.equals(a.get(i), b.get(i))) return a.get(i) - b.get(i);
            }
    ...
    public void backTracking(ArrayList<Integer> A, int sum, int idx, ArrayList<Integer> res)
    {
        if(sum == 0)
        {
        ...
        res.add(A.get(idx));
        backTracking(A, sum-A.get(idx), idx+1, res);
        res.remove(res.size()-1);

        while(idx + 1 < n && Objects.equals(A.get(idx), A.get(idx + 1)))
        {
            idx += 1;
        }
        backTracking(A, sum, idx+1, res);
