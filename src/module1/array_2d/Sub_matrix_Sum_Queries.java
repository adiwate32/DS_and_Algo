    ...
    public int[] solve(int[][] A, int[] B, int[] C, int[] D, int[] E) {
        ...
        for(int i=0; i<n; i++)
        {
            for(int j=0; j<m; j++)
            {
                ...
                {
                    psum[i][j] = (psum[i][j-1] + A[i][j] + mod) % mod;
                }
            }
        }
        ...
        {
            long sum = 0;
            sum += psum[D[i]-1][E[i]-1] % mod;
            if(B[i] > 1)
            {
                sum -= psum[B[i]-2][E[i]-1] % mod;
            }

            if(C[i] > 1)
            {
                sum -= psum[D[i]-1][C[i]-2] % mod;
            }

            if((C[i] > 1) && (B[i] > 1))
            {
                sum += psum[B[i]-2][C[i]-2] % mod;
            }
